(function attachDataStore(globalScope) {
  ...
  async function getState() {
    const [
      ...
      api.get("/inspections").catch(() => []),
      api.get("/issue-reports").catch(() => []),
      api.get("/resource-requests").catch(() => []),
      api.get("/progress-reports").catch(() => []),
      api.get("/budget-proposals").catch(() => []),
      api.get("/procurement-bills").catch(() => []),
      api.get("/qc-reviews").catch(() => []),
      api.get("/fund-releases").catch(() => []),
      api.get("/maintenance/schedules").catch(() => []),
      api.get("/maintenance/logs").catch(() => []),
      api.get("/field-assets/sensors").catch(() => []),
      api.get("/field-assets/materials").catch(() => []),
      api.get("/outcome-reports").catch(() => []),
